...
try { isRunning = false; if (elements.startPause) elements.startPause.textContent = 'Start'; } catch(_) {}
var msg = '[FAIL] ' + header;
var body = '';
try { body = '\n' + JSON.stringify(details, null, 2); } catch(_) { body = '\n' + String(details); }
try {
...
elements.testStatus.textContent = _baseStatusText() + ' | Eval: compile error: ' + (e && e.message ? e.message : String(e));
stopAndReport('Self-tests: Eval compile error', { expr: expr, error: (e && e.stack) ? e.stack : String(e) });
return;
}
...
var mismatches = 0, firstMsg = ''; var mod = found.table.length; var details = [];
...
var got; try { got = fn(n, sArr); } catch (e2) {
mismatches++; if (!firstMsg) firstMsg = 'runtime error on #' + j + ': ' + (e2 && e2.message ? e2.message : String(e2));
if (details.length < 20) details.push({ index:j, string:str, n:n, error: (e2 && e2.stack) ? e2.stack : String(e2) });
continue;
}
var idxEval = got & (mod - 1); var idxFunc = (found.hashes[j] & (mod - 1));
if ((idxEval|0) !== (idxFunc|0) || found.table[idxEval] !== j) {
mismatches++;
if (!firstMsg) firstMsg = 'mismatch on #' + j + ' (' + escapeCString(str) + '): eval=' + idxEval + ', func=' + idxFunc + ', table['+idxEval+']=' + found.table[idxEval];
if (details.length < 20) details.push({ index:j, string:str, n:n, got:got, idxEval:idxEval|0, idxFunc:idxFunc|0, tableAt:found.table[idxEval] });
}
}
...
if (mismatches > 0) {
stopAndReport('Self-tests: Eval mismatch', { expr: expr, tableSize: found.table.length, details: details });
}
}
...
if (!expr) { elements.testStatus.textContent = _baseStatusText() + ' | Editor: expr not found'; stopAndReport('Editor: expr not found', { codeSample: code.slice(Math.max(0, code.indexOf('int stringIndex')-80), Math.min(code.length, code.indexOf('int stringIndex')+120)) }); return; }
var fn;
try { fn = eval('(function(n,s){return (' + expr + ');})'); }
catch (e) { elements.testStatus.textContent = _baseStatusText() + ' | Editor: compile error: ' + (e && e.message ? e.message : String(e)); stopAndReport('Editor: compile error', { expr: expr, error: (e && e.stack) ? e.stack : String(e) }); return; }
var minLen = Infinity; for (var i = 0; i < strings.length; ++i) { var L = strings[i].length; if (L < minLen) minLen = L; }
...
var sArr = new Array(Math.max(padLen, n));
for (var k = 0; k < sArr.length; ++k) { var c = (k < n ? str.charCodeAt(k) : 0); if (c >= 128) c -= 256; sArr[k] = c; }
var got; try { got = fn(n, sArr); } catch (e2) { mismatches++; if (!firstMsg) firstMsg = 'runtime error on #' + j + ': ' + (e2 && e2.message ? e2.message : String(e2)); if (details.length < 20) details.push({ index:j, string:str, n:n, error: (e2 && e2.stack) ? e2.stack : String(e2) }); continue; }
var idxEval = got & (mod - 1); var idxFunc = (found.hashes[j] & (mod - 1));
if ((idxEval|0) !== (idxFunc|0) || found.table[idxEval] !== j) { mismatches++; if (!firstMsg) firstMsg = 'mismatch on #' + j + ' (' + escapeCString(str) + '): editor=' + idxEval + ', func=' + idxFunc + ', table['+idxEval+']=' + found.table[idxEval]; if (details.length < 20) details.push({ index:j, string:str, n:n, got:got, idxEditor:idxEval|0, idxFunc:idxFunc|0, tableAt:found.table[idxEval] }); }
}
...
if (mismatches > 0) {
stopAndReport('Editor: mismatch', { expr: expr, tableSize: found.table.length, details: details });
}
}
...
if (zeroTerminated) {
code = code.replace(/return\s*\(stringIndex\s*>=\s*0\s*&&\s*strncmp\([^;]+;/m, "return (stringIndex >= 0 && strcmp(s, STRINGS[stringIndex]) == 0) ? stringIndex : -1;");
} else {
code = code.replace(/return\s*\(stringIndex\s*>=\s*0\s*&&\s*strcmp\([^;]+;/m, "return (stringIndex >= 0 && strncmp(s, STRINGS[stringIndex], n) == 0 && STRINGS[stringIndex][n] == 0) ? stringIndex : -1;");
}
} catch(_) {}
return code;
}
...
var ok = 0, fail = 0, failures = []; function T(name, fn){ try { fn(); ok++; } catch(e){ console.error("Test fail:", name, e); failures.push({ test:name, error:(e && e.stack) ? e.stack : String(e) }); fail++; } }
...
T('match HASH_TABLE brackets', function(){ var line = 'int stringIndex = HASH_TABLE[((s[1] + n) & 31) ^ (9 < n ? s[9] : 0)];'; var i = line.indexOf('['); var j = findMatchingSquare(line, i); if (j !== line.length - 2) throw new Error('mismatch '+j); });
T('extract editor expr', function(){ var code = 'int stringIndex = HASH_TABLE[(n ^ s[0]) & 15];'; var expr = extractEditorHashExpr(code); if (!expr || expr.indexOf('(n ^ s[0]) & 15') < 0) throw new Error('extract failed: '+expr); });
...
if (fail > 0) { stopAndReport('Self-tests failed', { summary: msg, failures: failures }); }
}

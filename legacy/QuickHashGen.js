		...
		if (s[i] === "\\") {
			...
			if (index >= 0) o += C_ESCAPE_CODES[index];
				...
				switch (c) {
					...
					case "u":
						while ("0123456789abcdefABCDEF".indexOf(s[i]) >= 0 && i < b + 4)
							++i;
...
function QuickHashGen(
		...
		if (cpp) {
			...
			OPS = [
				...
				[
					...
					1,
					function () {
						return rnd.nextInt32() & constantMask;
					},
				],
				[
					4,
					...
					1,
					function () {
						return "p[" + rnd.nextInt(safeLength) + "]";
					},
				],
				[
					4,
					...
					2,
					function () {
						var i = rnd.nextInt(maxLength - safeLength) + safeLength;
						return "(" + i + " < n ? p[" + i + "] : 0)";
					},
				],
				[
					...
					Infinity,
					function (c) {
						return "0u + " + rndExpr(c - 1, 1) + " << " + (rnd.nextInt(31) + 1);
					},
				],
				[
					1,
					...
					Infinity,
					function (c) {
						return rndExpr(c - 1, 1) + " >> " + (rnd.nextInt(31) + 1);
					},
				],
				[
					2,
					...
					function (c) {
						...
						return rndExpr(b, 2) + " + " + rndExpr(c - b, 2);
					},
				],
				[
					2,
					...
					function (c) {
						var b = rnd.nextInt(c - 1) + 1;
						return rndExpr(b, 2) + " - " + rndExpr(c - b, 3);
					},
				], // that's right, right-hand side of - must have one higher prec to avoid x - (y - z) becoming x - y - z
				[
					0,
					2,
					Infinity,
					function (c) {
						var b = rnd.nextInt(c - 1) + 1;
						return rndExpr(b, 0) + " ^ " + rndExpr(c - b, 0);
					},
				],
				[
					3,
					...
					function (c) {
						var b = rnd.nextInt(c - 1) + 1;
						return rndExpr(b, 3) + " * " + rndExpr(c - b, 3);
					},
				],
			];
		} else {
			...
			OPS = [
				...
				[
					...
					1,
					function () {
						return "w[" + rnd.nextInt(safeLength) + "]";
					},
				],
				[
					4,
					...
					function () {
						return (
							"w[" + (rnd.nextInt(maxLength - safeLength) + safeLength) + "]"
						);
					},
				],
				[
					...
					Infinity,
					function (c) {
						return "0 + " + rndExpr(c - 1, 1) + " << " + (rnd.nextInt(31) + 1);
					},
				],
				[
					1,
				...
				[
					...
					function (c) {
						var b = rnd.nextInt(c - 1) + 1;
						return (
							"Math.imul(" + rndExpr(b, 0) + ", " + rndExpr(c - b, 0) + ")"
						);
					},
				], // 0 prec here to avoid any ( )
			];
		}
	...
	this.search = function (complexity, iterations) {
		...
		for (var i = 0; i < iterations; ++i) {
			...
			var expr = generateRandomExpression(
				globalPRNG,
				complexity,
				maxTableSize - 1,
				false,
	...
	this.generateCOutput = function (template, foundSolution) {
		var cExpression = generateRandomExpression(
